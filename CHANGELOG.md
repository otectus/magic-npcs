...
## [0.9.1] — Casting correctness and validation

A correctness patch. Every casting entry point — the AI goal, the detached driver, the CustomNPCs script API and the Easy NPC `magicnpcs:cast` action — now passes the same mandatory validation before anything is written, a spell has one identity rather than one per spelling, a failing spell can no longer leave a caster stuck mid-channel, and a deliberately enormous cooldown can no longer wrap into the shortest one allowed. No new gameplay features, no config migration, no save-data change. This release closes a scoped set of findings from `docs/MAGIC_NPCS_AUDIT_AND_ROADMAP.md`; see **Known limitations** for what it does not close.

### Fixed

- **Enforce NPC-framework eligibility across assignment and casting entry points** (MN-002). The CustomNPCs bridge deliberately refuses to activate against a build it is not pinned to, but through 0.9.0 that pin gated only the *adapter*: adapter resolution fell back to a generic default, so an NPC on an unsupported build still reached the reconciler's assignment path and the manual school writer, and was managed anyway. A new neutral `FrameworkEligibility` gate under `core/caster` is consulted before any write — before a school is saved, before attributes, equipment or goals are touched, before `caster_chance` is rolled, and before a cast is prepared. New reasons: `ReconcileResult.ReasonCode.UNSUPPORTED_NPC_FRAMEWORK` / `NPC_FRAMEWORK_UNAVAILABLE` / `NPC_FRAMEWORK_DISABLED`, `SchoolAssignResult.UNSUPPORTED_NPC_FRAMEWORK` / `NPC_FRAMEWORK_UNAVAILABLE` / `NPC_FRAMEWORK_DISABLED`, and script result code `UNSUPPORTED_NPC_BUILD` (appended last, so existing numeric codes are unchanged). Vanilla mobs, Recruits and Easy NPC are not gated by this; only the exactly-pinned framework is.
- **Apply shared mandatory validation to automatic and external requests** (MN-003). A side-effect-free `CastRequestValidator` now runs framework eligibility, the spell allow-list, the mob-cast manifest, activity permission, target identity/world/liveness/relationship, line of sight, the canonical cooldown and mana — for the AI goal and for every external caller alike. It prepares no spell, creates no managed state, spends nothing, publishes no event and consumes no RNG, so `canCast` and `cast` cannot give different answers. Spell levels are bounded to the spell's own declared minimum and maximum on **both** routes before Iron's reads them for mana, cast time or effect magnitude; the AI's `effectiveLevel` previously returned an authored floor unchecked. An Easy NPC `magicnpcs:cast` action aimed at a target with no valid target is now refused and logged instead of silently becoming a self-cast. Weighted selection, cast chance, wind-up and per-entry conditions remain AI policy and are still bypassed by an explicit scripted cast.
- **Clean up failed cast sessions safely and preserve acceptance accounting** (MN-004). `MobCastSession` now tracks preparation, acceptance, effect entry and termination explicitly. Every Iron's hook is guarded: a spell that throws in `checkPreCastConditions`, `onServerPreCast`, `onServerCastTick`, `onCast`, `onServerCastComplete` or `shouldAIStopCasting` ends that one cast with a single diagnostic naming the spell and stage, and leaves the caster usable — rather than leaving Iron's believing it is still casting, which blocked every later cast on that mob. Cleanup is at-most-once and terminal state is taken **before** the externally re-entrant terminal announcement. Entry into an effect hook is recorded before the hook is called, so an effect that lands and then throws is never refunded and retried. A third-party listener that throws while handling a cast event no longer takes the throw back through the session; the veto path deliberately keeps Forge's own behaviour, so a broken veto cannot be read as consent.
- **Share spell identity, cooldowns and overrides across accepted aliases** (MN-005). `irons_spellbooks:devour`, `minecraft:devour` and a bare `devour` are one spell. Cooldowns are keyed by the canonical registry id, retained keys are compared canonically across a loadout change, and cast events publish the canonical id. A loadout listing the same spell twice under different spellings keeps the first entry in author order and drops the later one with a warning naming both spellings — and a second warning when their cooldown or level overrides disagree, since the first entry's values are what apply. A caster's existing cooldowns are normalised on reconcile, merging equivalent keys onto the **longest** remaining deadline.
- **Handle callback-created detached casts and teardown safely** (MN-013). Completion and cancellation are dispatched synchronously, so a listener may answer one by casting again. `DetachedCastDriver` now stages additions and removals across tick and notification boundaries instead of mutating the list being iterated, refuses to drain recursively, and gives a callback-created session its first advance at the next tick boundary rather than a second advance in the same tick. New detached casts are declined during teardown (`Refusal.SHUTTING_DOWN`), and the guard is scoped to one server lifetime so an integrated client's next world is not left without detached casting. `MagicNpcs.onServerStopped` terminates live sessions **before** clearing the event and managed-state bookkeeping; the previous order cleared the open-cast records first, so every terminal event fired during shutdown was silently dropped.
- **Prevent large cooldown values from wrapping into faster casting** (MN-017). The roadmap's witness: a 30,000,000-tick base at `cooldown_multiplier: 100` is 3×10⁹, the narrowing cast wrapped it negative, and the floor then replaced it with `balance.minCooldownTicks` — a large positive cooldown made the spell cast *more* often. `CooldownResolver` now saturates to `Integer.MAX_VALUE` before narrowing and refuses non-finite or negative multipliers; `ManagedCasterState` holds deadlines as `long` values on the level game-time clock, so the saturation is not merely relocated into the deadline arithmetic. Moving off `mob.tickCount` also fixes a latent bug of its own: that counter restarts at zero when an entity reloads, so a deadline written before an unload read as a very long rest afterwards. Cooldowns remain in memory only, with unchanged unload semantics and no persistence. `cooldown_multiplier` is now validated in the parser (`COOLDOWN_MULTIPLIER_INVALID` error for non-finite or negative, `COOLDOWN_MULTIPLIER_SUSPICIOUS` warning above 1000) — the old `Math.max(0.0, x)` clamp did nothing to `NaN`.
- **Correct segment-to-hitbox clearance calculations** (MN-011). `LineOfFire.distanceToSegmentSqr` projected the protected entity's box *centre* onto the fire line, so a line that passed through the box away from its centre could measure clear. It now computes the exact minimum distance between the segment and the axis-aligned box (zero whenever the segment enters it). Regression: `LineOfFireTest` gains the roadmap's REG-20 certificate (`a = (-5, -4, 0.5)`, `b = (15, 14, 0.5)`, box `[(0,0,0),(10,1,1)]`, radius 1.5, previously reported clear, now blocked) plus reference distances, degenerate segments, box shapes, translation and endpoint-reversal invariance and 200 seeded comparisons against a brute-force oracle.

### Build and testing

- **Correct Iron's compile-artifact provenance while retaining the dependency pin** (MN-001). `irons_spellbooks_compile` claimed `1.20.1-3.16.3`; the pinned CurseForge file id resolves to an artifact whose own `mods.toml` and manifest both say `1.20.1-3.15.0`. The label is corrected, the file id is unchanged, and the separate fact it had been conflated with — the Iron's build the shipped spell manifest's classifications came from — now lives in its own `irons_spellbooks_manifest` property. The jar's `MagicNPCs-Irons-Verified` manifest attribute, which implied a verification no build step performed, is replaced by `MagicNPCs-Irons-Compiled-Against` and `MagicNPCs-Irons-Manifest-Reference`.
- **`checkArtifactProvenance`** (new Gradle task, wired into `check`) compares every declared dependency label against the artifact's own metadata, records SHA-256 for the published artifact and separately for ForgeGradle's remapped derivative, re-checks the CustomNPCs pinned hash, and fails if any optional mod is declared on a configuration that would bundle it. A deliberately wrong label fails it, via `-Pirons_spellbooks_compile=deliberately-wrong`. **`writeBuildBaselineReport`** records checkout, toolchain, resolved compile graph and local JAR hashes.
- **Separate absence smoke checks from required positive integration tests** (MN-015). Through 0.9.0 every integration GameTest opened with "if the host mod is absent, succeed", so a completely green report was indistinguishable from one in which nothing ran. With no profile selected nothing changes and the offline run still proves the soft-dependency path. `-PtestRuntimeProfile=<name>` selects a profile (`irons-floor`, `irons-dev`, `irons-manifest`, `irons-easy`, `irons-customnpcs`, `irons-recruits`, `combined`) that names the mods which must be loaded; a test whose host is missing then fails naming it. The new `checkPositiveGameTestPreflight` task fails before the server boots when a profile's dependencies are not staged, and `verifyPositiveGameTests` rejects a run with zero executed positive tests, any failure, or any skipped positive path. Executed, skipped and failed counts are reported separately.
- **`CustomNpcsCompatGameTests` strengthened.** `spawnNpc` now fails, naming the reason, when CustomNPCs is loaded but its entity type is missing or does not spawn a `Mob`. `anAcceptedCastAnnouncesOneStartAndOneEnding` treats a refused expected-positive cast as a failure under a profile that requires CustomNPCs, and additionally asserts that acceptance spent mana and started a cooldown. `casterGoalSurvivesAiRebuild` now observes an accepted, mana-spending cast before *and* after the AI rebuild, and asserts foreign goals are unchanged — a repaired goal that cannot cast is not a repaired caster.
- **New regression tests.** Unit: `FrameworkEligibilityTest` (REG-06, every gate branch plus vanilla/Recruits/Easy positive controls), `PositiveProfileTest` (REG-35), `CooldownResolverTest` and `CastTimeResolverTest` additions (REG-23: the roadmap witness, integer boundaries, NaN/infinity, negative multipliers, rounding, saturation, and positive controls for the ordinary range), `ManagedCasterStateTest` additions (REG-01/REG-23: long deadlines, alias merge on the longest deadline, canonical retention), `LoadoutParseTest` additions (`cooldown_multiplier` validation). GameTest: `anAliasAndItsCanonicalFormShareOneCooldown` (REG-01), `competingRequestsProduceAtMostOneAcceptedSession` (REG-03), `callbackCreatedCastDoesNotCorruptTheDetachedDriver` (REG-05).

### Known limitations

- **This is a scoped patch, not full roadmap verification.** Several P1 findings are deliberately deferred and remain open. Nothing here should be read as "the integration is verified"; the release closes the findings listed above and no others.
- **Deferred (Phase 2):** stable spell selection across reconciles (MN-006), owned-goal repair (MN-008), foreign retaliation ownership (MN-009), second-world reactivation of the NPC bridges (MN-010), and unchanged-reload behaviour (MN-014). The detached driver's new teardown guard is scoped per server lifetime, which is necessary for that guard alone and is not a fix for MN-010.
- **Deferred (Phase 3, remainder):** support-spell recipient strategy (MN-007) and live collateral/focus checks at effect emission (MN-012). Accepted casts stay bound to the recipient they were authorised for; nothing retargets them.
- **Deferred (Phase 4, beyond these release notes):** the wider documentation closure (MN-016).
- **Deferred (Phase 5):** all enhancements (MN-E01 … MN-E08). The suspected integration issues MN-R01 … MN-R07 remain investigations, not asserted fixes.
- **Fault injection is asserted by construction, not by a running spell.** The exception-safety work (MN-004) is written against every Iron's hook, but the regression suite has no spell that deliberately throws; those paths are covered by review and by the at-most-one-terminal assertions, not by an executed fault.
- **LUMINOUS: BEASTS remains unverified.** It is staged by `-PluminousRuntime` for investigation only; no compatibility is claimed.
- **Launch-time protection is not universal friendly-fire immunity.** Request validation and clearance geometry decide whether a cast may start and where it is aimed. They do not certify what a projectile, chain, field or summon does afterwards — that remains Iron's own ownership rules.
- **Profiles tested by this release.** The unit suite and a clean compile were run. The required-positive GameTest profiles are infrastructure delivered by this release; their runs are recorded separately and are not asserted here.

## [0.9.0] — Native cast-time override and casting-driver observability

Per-spell native Iron's cast-duration control via `cast_time` (absolute ticks) and `cast_time_multiplier` (scaling), with exact backward compatibility when both are omitted. The casting goal heartbeat now lives on managed state and is surfaced in `/magicnpcs why` output, enabling tooling to detect when a mob's AI is not running the vanilla goal selector. Out-of-combat SUPPORT casting gains comprehensive test coverage (no behaviour change). The `-PluminousRuntime` dev profile stages LUMINOUS: BEASTS for integration verification.

### Added

- **Layered spell support and provenance.** Spells now have a five-tier support chain with explicit provenance in diagnostics: (1) `spells.capabilityOverrides` config entries; (2) datapack spell manifests (`data/<namespace>/spell_manifests/<name>.json`); (3) the built-in Iron's table (reviewed against the version this build was tested against); (4) `spells.trustedNamespaces` (treat-as-ADDON_DEFAULT for each spell, with caveats); (5) nothing (UNVERIFIED). `/magicnpcs spells` output now includes a one-letter provenance column `V O M T U` with legend. ADR 0012.
- **Datapack spell manifests.** Pack authors can declare spells' mob-cast capabilities without touching Magic NPCs' config: `data/<any-namespace>/spell_manifests/<name>.json` with format `{"format": 1, "verified_against": "<free text>", "spells": {"ns:path": "CAPABILITY", ...}}`. Capabilities: `DIRECT`, `TARGET_ENTITY`, `TARGET_AREA`, `GROUND_AOE_FORWARD`, `SUMMON`, `ADDON_DEFAULT`, `MULTI_TARGET`, `SPECIAL_PREPARATION`, `PLAYER_ONLY`, `UTILITY_NON_COMBAT`, `UNVERIFIED`. Keys starting with `_` are ignored. Files are merged in resource-id order, and a later file overrides an earlier one with a logged warning (`SpellManifestLoader`). Reloadable on `/reload`, registered before loadout manager so the loadout validation sees new manifests immediately. Problem codes: `MANIFEST_FORMAT`, `MANIFEST_NO_SPELLS`, `MANIFEST_BAD_ID`, `MANIFEST_BAD_CAPABILITY`.
- **`spells.trustedNamespaces` and `spells.capabilityOverrides`.** Two new config settings in `[spells]` block (server config, auto-synced): `trustedNamespaces` (list of namespaces like `traveloptics`, no colon) treats spells from those namespaces as `ADDON_DEFAULT` when no higher layer covers them — a weak claim, not verified, used for single-target add-on spells that work without a manifest. `capabilityOverrides` (list of `ns:path=CAPABILITY` strings like `traveloptics:tidal_lance=TARGET_ENTITY`) outrank manifests and the built-in table, so one spell can be fixed without a datapack. Both config changes apply on reload (trigger the existing `ModConfigEvent.Reloading` handler).
- **Spell-id wildcard filters.** `spells.spellWhitelist` and `spells.spellBlacklist` now accept `namespace:*` wildcard patterns in addition to exact ids (e.g., `traveloptics:*` to block a whole namespace). A non-empty whitelist without wildcards still excludes spells from `spells.trustedNamespaces`; `/magicnpcs validate` prints `Spell support: <N> verified, <N> namespace_trusted, <N> unverified; <N> excluded by spells.spellWhitelist/spellBlacklist` summarizing provenance and filter exclusions.
- **Diagnostic codes and `/magicnpcs why` enhancements.** New codes in `/magicnpcs validate` output: `[NAMESPACE_TRUSTED]` (warning when a spell is claimed by namespace trust only, not verified; friendly-fire uses CORRIDOR shape, no cast-data guarantee; add it to a manifest to silence) for addon spells. `/magicnpcs why` goal table is now sorted by priority then class name (stable under mods that replace goals); header row includes `native-attack` column showing `exact`, `pattern:<regex>`, or `-` for goal matches; after the table, `candidate attack goals not matched: <names>` plus hint to add to `general.suppressibleAttackGoals` or `general.attackGoalNamePatterns`. Info line added for brain AI: `this mob also runs a Brain (<activity>); look/movement may be contested outside the goal selector [BRAIN_AI]`. Mana clamp warning: `max_mana clamped to N (wanted N; attribute range max N) — an attribute range cap applies; see AttributeFix / Apothic Attributes [MANA_CLAMPED]` when Iron's attribute caps conflict with configured mana.
- **`general.attackGoalNamePatterns`.** New config key (default `Attack, Ranged, Shoot, Bow, Crossbow, Gun, Spit, Breath, Charge, Cast`; case-sensitive regexes, matched with `find()` against goal's innermost simple class name only) extends the exact-name list `general.suppressibleAttackGoals` (which also recognizes nested `Outer$Inner` form). Patterns are applied only to goals that are not target goals, not Magic NPCs' own, and declare `MOVE` or `LOOK`, enabling `native_attack: suppress/yield` to work with modded mobs without naming every class. When a goal is matched by pattern, the `/magicnpcs why` native-attack column shows `pattern:<regex>`. Behaviour note: loadouts using `suppress`/`yield` on modded mobs may now suppress/yield to goals that were previously invisible; add a `native_attack: coexist` override per-entity if that is wrong for a specific mob.
- **Per-spell native cast-time override.** Two optional loadout fields control how long an NPC's Iron's spell takes to charge:
  - **`cast_time`** (ticks): absolute override, highest precedence. `"cast_time": 6` makes this spell cast in exactly 6 ticks, ignoring Iron's default.
  - **`cast_time_multiplier`** (number): scales Iron's effective cast time. `0.5` halves it, `2.0` doubles it. Ignored if `cast_time` is set.
  - Both fields are optional; omitting both means exact backward compatibility (Iron's timing is used as-is).
  - Applies to LONG and CONTINUOUS spells only; INSTANT/NONE spells ignore both fields (see `/magicnpcs validate` warning `CAST_TIME_IGNORED` when set on an INSTANT spell).
  - Multiplier applies to Iron's effective cast time (including caster-side modifiers), not the base: a caster with speed boost sees the multiplied value of the boosted time.
  - Parser rejects malformed values (`cast_time` non-integer or negative → `CAST_TIME_NEGATIVE` error; `cast_time_multiplier` non-number or negative → `CAST_TIME_MULTIPLIER_INVALID` error) as file-level errors, not clamped/silenced.
  - When both fields are present, info-level log `CAST_TIME_ABSOLUTE_WINS` ("cast_time overrides cast_time_multiplier for this spell") and the entry keeps both values.
  - For LONG spells the cast duration is time-to-effect release; for CONTINUOUS spells, it is also the channel length. Neither field changes the lifetime of an effect spawned *after* casting completes.
  - Gravity Fissure example (15-tick native cast, 3.16.3): `"cast_time_multiplier": 0.5` resolves to 8 ticks; `"cast_time": 6` resolves to 6 ticks (absolute). The black hole's lifetime comes from spell power, not cast duration.
  - **Before 0.9.0 there was no native cast-duration syntax.** `"windup": 0` only removes Magic NPCs' own pre-cast delay; it cannot shorten Iron's native cast. The two phases are distinct: windup → Iron's cast/channel → effect.
- **Session double-tick guard.** `MobCastSession.tick()` now ignores a second call in the same game tick, guarding against edge cases where both the goal's `start()` and `GoalSelector.tickRunningGoals(true)` advance the session on the same tick. **One-tick consequence:** A `windup: 0` LONG/CONTINUOUS cast previously advanced twice on its start tick and now advances once, so such casts complete one tick later than in 0.8.0. INSTANT casts are unaffected.
- **Casting-driver observability.** `ManagedCasterState` now carries `heartbeat(now)` and `goalHeartbeatAge(now)` fields, stamped by the goal at the top of `canUse()` and `tick()`. `/magicnpcs why` output now includes `casting driver: goal` and `goal heartbeat: N tick(s) ago` (or `never`). When the age exceeds 40 ticks, a blocker is added: `injected casting goal has not been evaluated for N ticks — this mob's AI may not run the vanilla goal selector [GOAL_NOT_EVALUATED]`. This is the signal for a modded mob whose AI bypasses the vanilla goal selector, requiring a fallback cast driver.
- **Validate diagnostic codes:**
  - `CAST_TIME_NEGATIVE` — `cast_time` is less than zero (file-level error, rejects loadout record).
  - `CAST_TIME_MULTIPLIER_INVALID` — `cast_time_multiplier` is not finite or is less than zero (file-level error).
  - `CAST_TIME_ABSOLUTE_WINS` — both `cast_time` and `cast_time_multiplier` are set on the same spell (info, no error).
  - `CAST_TIME_IGNORED` — either field is set on an INSTANT or NONE spell; both are ignored at runtime (warning in `/magicnpcs validate`).
  - `GOAL_NOT_EVALUATED` — goal heartbeat age exceeds 40 ticks in `/magicnpcs why` output.
- **`/magicnpcs why` state section** now prints `casting driver: goal` and `goal heartbeat: N tick(s) ago` (or `never`).
- **`/magicnpcs loadout entity` spell rows** now print `cast_time=<N>t` and/or `cast_time_multiplier=<M>x` when set (omitted when not set, so pre-0.9.0 loadouts read exactly as before).
- **Out-of-combat SUPPORT casting gains regression coverage** (no behaviour change): new unit tests (`SpellEligibilityTest`, `ManagedCasterStateTest`, `CastTimeResolverTest`) and GameTests (`woundedIdleCasterSelfHeals`, `supportOutOfCombatDisabledNeverCasts`, `targetAcquisitionUsesCombatCadence`, `outOfCombatHealSkipsTelegraph`, `outOfCombatHealWaitsForMana`) verify the existing feature's contract. GameTests also verify cast-time override and session guard edge cases (`gravityFissureHalfMultiplierCastsInEightTicks`, `gravityFissureAbsoluteSixWithWindupSix`, `noOverrideKeepsIronsEffectiveDuration`, `scriptedCastIgnoresLoadoutOverride`, `sameTickDoubleTickIsIgnored`). Tests written but not executed in the bare dev environment (full `-PdevRuntime` required).
- **`/magicnpcs audit spells` empirical spell testing.** Two-phase in-game audit (permission level 2; `command/AuditCommand.java`, `integration/irons/SpellAuditRun.java`): RESOLVE mode (default) checks every spell for resolution, provenance, enabled status, cast type, mana cost, effective cast time, and `checkPreCastConditions` calls with each Iron's call guarded. CAST mode (opt-in with literal `cast` keyword; prints a disposable-world warning) additionally runs each spell through a real `MobCastSession` with a 2-tick resolved duration on two spawned no-AI invulnerable zombie dummies, recording mana and nearby-entity deltas and `NO_OBSERVABLE_EFFECT` hint. One run per server, command filters by optional `[namespace]`. Outcomes: OK, PRECAST_REFUSED, DISABLED, EXCEPTION, REFUSED, LIFECYCLE_COMPLETED, CANCELLED, TIMEOUT. Report written to `<gamedir>/magicnpcs/audit-spells-<timestamp>.txt` and `.json` with header (versions, mode, filter), table `id | provenance | cast type | mana | outcome | manaD | entD | detail`, and per-outcome/per-namespace counts. Proves lifecycle, not usefulness; dummies removed on completion, cancel, or server stop. `/magicnpcs audit status` and `/magicnpcs audit cancel` manage the run.
- **`tools/spell_manifest_audit.py` offline heuristic auditor.** Stdlib Python 3 script for modpack authors to draft manifests: `python tools/spell_manifest_audit.py <mods_dir> --out <out_dir> [--irons <jar>] [--namespace ns ...] [--verbose]`. Reads class constant pools (no JVM), finds transitive `AbstractSpell` subclasses, resolves ids from lang keys, infers cast type and heuristic capability with confidence and evidence. Outputs `<ns>.manifest.json` (draft with `_review` list), `<ns>.audit.md`, `summary.md` with calibration against built-in Iron's table (on this machine's instance: 114 Iron's spells, 91.2% exact match). Drafts are heuristic, not runtime-verified; `verified_against` says so. Known limitations: GROUND_AOE_FORWARD is never inferred; re-skinned spells sharing a parent's lang key and multi-id proxy classes (e.g., `ars_n_spells:ars_cross_*`) cannot be resolved.
- **`ADDON_DEFAULT` opportunistic target data.** The new `SpellCapability.ADDON_DEFAULT` enum value marks namespace-trusted spells. Treated as `DIRECT` for friendly-fire geometry (CORRIDOR) and target requirement (not required), but `MobCastSession.begin` (~:183-189) installs target cast data opportunistically when a live target exists, making single-target add-on spells work without a manifest.
- **`-PluminousRuntime` dev profile** (opt-in): loads LUMINOUS: BEASTS V1.2.7 (Forge 1.20.1) at runtime for integration verification. runtimeOnly → never bundled or compiled against. Additive to `-PdevRuntime` (Luminous requires GeckoLib, which `-PdevRuntime` already stages). Luminous Phoenix and Witch Doctor compatibility is **instrumented in 0.9.0 but NOT yet runtime-verified** — the feature is designed but deliberately not implemented until a runtime run shows `[GOAL_NOT_EVALUATED]` on a real mob. Pinned by CurseForge file id (`gradle.properties` lines 46–52).
- **Loadouts for absent mods are skipped, not rejected.** When a loadout's `entity_type` or `profession` namespace is not installed, or when every spell entry belongs to absent mods, the file becomes SKIPPED (status `INAPPLICABLE`) with INFO-level problems only — not an error. This distinguishes "the mod is not installed" from "the author wrote something wrong": `/magicnpcs validate` reports the count and path, the file is invisible at runtime, and upgrades that install the mod automatically enable it. See `core/loadout/RegistryChecks.java` (interface: `modLoaded`), `LoadoutParser` (~:250-256 for entity type, ~:285-289 for profession, ~:327-330 for spell entries), `LoadoutRecord.Status.INAPPLICABLE` (~:50-57), and `LoadoutCatalog.Counts` (~:100-121).
- **Iron's target-raycast facing fix.** Iron's `Utils.preCastTargetHelper` raycasts along the caster's **current facing**, not from pre-installed target cast data. Target spells were therefore refusing a caster that had not turned yet. New `core/caster/CasterFacing.snap(Mob, LivingEntity)` (~:27-42) snaps the caster's yaw/pitch at the target's eyes; `MobCastSession.prepare(caster, target, spell, level)` (~:236-267) calls it before the pre-cast check when a target is required and present. **Behaviour change:** SUPPORT-role target spells (e.g., `blessing_of_life`) now also get the facing snap, previously ATTACK-role only. Record `Prepared(data, ownsCastData, passed, refusal)` (~:220) captures the outcome for testing and auditing.
- **Audit fidelity: new outcomes and refusal classification.** `/magicnpcs audit spells` RESOLVE mode now sets dummy targets and probes via `MobCastSession.prepare`, adding outcomes `EXPECTED_PLAYER_ONLY` (detail "player-only spell; expected for a mob caster") and `PRECAST_REFUSED` with detail "target raycast missed (manifest: TARGET_ENTITY)" for target spells. When a VERIFIED supported capability refuses, `[MANIFEST_SUSPECT]` is appended to the detail. Summary adds `refusals contradicting the manifest: N` (ids listed) and `expected player-only refusals: N`; header prints the manifest reconciliation summary and `WARNING: dummies have no line of sight; target-spell refusals are not meaningful` when applicable. Pure classifier: `core/audit/RefusalClassifier.java` (test `RefusalClassifierTest`).
- **Manifest reconciliation logs stale and unlisted spells.** Removed the manifest rows `soulfire_ray` and `thunder_step`, which have no registered spell in Iron's 3.16.3; the manifest is now reconciled against the live registry at server start (`manifest vs registry` in `/magicnpcs config`). New `core/spell/ManifestReconciliation.diff` compares both directions, excluding the `none` placeholder, and produces summary `manifest: <rows> rows, <n> unregistered, <m> unlisted`. Runs from `integration/irons/ManifestReconciler.runOnce()` on `ServerStartedEvent` (FORGE bus, ~:111-113): WARN per stale row ("remove it or bump VERIFIED_AGAINST"), INFO per unlisted registered spell ("it is unverified until one is added"). See `ManifestReconciliationTest`.
- **GameTest `targetSpellStartsWhenCasterFacesAway`.** Verifies the facing snap: a caster spawned facing away from its target starts a target spell cast (it has 2 ticks to turn; the spawn packet hasn't landed yet). Holder: `IronsCastingTests` (~:1556).
- **Validate command tally fix.** `/magicnpcs validate` tallies now correctly separate fatal errors from warnings. `LoadoutCommand.validateSpells` now returns `{fatal, warning}` counts so the result line matches what was printed. Previously, a non-fatal spell problem (WARN-level) was counted as a fatal error in the output tally, causing `FAILED` when only warnings were present. (`command/LoadoutCommand.java` ~:406-441)
- **Manifest corrections from Iron's 3.16.3 bytecode verification.** Three spells in `SpellManifest.java` have corrected capability mappings with verified rationale (`integration/irons/SpellManifest.java`):
  - **`haste` TARGET_ENTITY → DIRECT:** Raycasts for an ally via `Utils.shouldHealEntity` and falls back to self-casting when no healable entity is hit, so a mob can safely self-cast it as a SUPPORT (self-heal/buff) spell without buffing an unintended target.
  - **`sacrifice` TARGET_ENTITY → SPECIAL_PREPARATION:** Only accepts the caster's own `IMagicSummon` entities as targets, not arbitrary entities.
  - **`wololo` TARGET_ENTITY → UTILITY_NON_COMBAT:** Only accepts a Sheep as its target (recolours wool); never useful for combat.
- **Audit outcome `EXPECTED_UNSUPPORTED`.** A refused spell whose verified manifest capability is not mob-supported (e.g., `SPECIAL_PREPARATION`, `UTILITY_NON_COMBAT`, `MULTI_TARGET`) is now reported as expected with detail "manifest marks this <CAPABILITY>; a mob cannot supply it" instead of `PRECAST_REFUSED` with a false alarm flag. This distinguishes manifest capabilities that legitimately cannot work (`EXPECTED_UNSUPPORTED`) from genuine bugs in the audit probe or manifest (`PRECAST_REFUSED_SUSPECT` when a verified supported capability refuses). Classifier: `core/audit/RefusalClassifier.java` (~:19-64). Summary line added: `expected unsupported refusals: N` (in `integration/irons/SpellAuditRun.java` ~:609-610).

### Changed

- **No functional change to out-of-combat SUPPORT.** ADR 0005 semantics are preserved: ATTACK never eligible without a target; SUPPORT eligible without a target only when hurt or a matching condition has no self-health gate; idle self-heals play no telegraph. Config `balance.supportOutOfCombat` and tick cadence remain as-is.

### Known limitations

- **Luminous compatibility is designed but deferred.** Luminous: Beasts Phoenix and Witch Doctor are instrumented but a shared goal+fallback-driver architecture is deliberately not deployed until a runtime test on the actual mod surfaces `[GOAL_NOT_EVALUATED]`, proving the need.
